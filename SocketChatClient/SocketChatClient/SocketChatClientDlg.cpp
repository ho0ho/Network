...
void CSocketChatClientDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//  DDX_IPAddress(pDX, IDC_IPADDR, m);
	DDX_IPAddress(pDX, IDC_IPADDR, m_ipaddr);
...
BOOL CSocketChatClientDlg::OnInitDialog()
{
	...
	if (connectSock == INVALID_SOCKET) {
		AfxMessageBox(_T("Failed to create connectSock"));
		return -1;
	}	
...
void CSocketChatClientDlg::OnBnClickedButtonSend()
{
	...
	UpdateData();
	//send
	/*WideCharToMultiByte(CP_ACP, 0, m_msg, -1, )*/
	int retval = send(connectSock, (CStringA)m_msg, lstrlen(m_msg), 0);
...
void CSocketChatClientDlg::OnBnClickedButtonConnect()
{
	...
	serveraddr.sin_addr.s_addr = htonl(m_ipaddr);
	//inet_addr(/*"192.168.1.15"*//*"127.0.0.1"*/);
	serveraddr.sin_port = htons(/*8192*/40100);

	int retval = connect(connectSock, (SOCKADDR*)& serveraddr, sizeof(serveraddr));
	if (retval == SOCKET_ERROR) {
		AfxMessageBox(_T("Failed to connect ConnectSock"));
		return ;
	}
